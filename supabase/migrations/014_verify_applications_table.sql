-- Verify and create applications table if needed
-- This migration ensures the applications table exists with all necessary fields and policies

-- Create applications table (IF NOT EXISTS ensures it won't fail if already present)
...
-- Add trigger for updated_at (drop and recreate to ensure it's correct)
...
-- Allow anyone (including anonymous users) to submit applications (insert)
